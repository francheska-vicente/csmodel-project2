...
class CollaborativeFiltering(object):
    ...
    def get_cosine_similarity(self, vector1, vector2):
        ...
        if(isinstance(vector1, pd.Series) and isinstance(vector2, pd.Series)):
            return np.sum(vector1*vector2)/(np.sqrt(np.sum(np.square(vector1)))*np.sqrt(np.sum(np.square(vector2))))
        elif(isinstance(vector1, pd.Series) and isinstance(vector2, pd.DataFrame)):
            return np.sum(vector1*vector2, axis=1)/(np.sqrt(np.sum(np.square(vector1), axis=0))*np.sqrt(np.sum(np.square(vector2), axis=1)))
        else:
            return np.sum(vector1*vector2, axis=1)/(np.sqrt(np.sum(np.square(vector1), axis=1))*np.sqrt(np.sum(np.square(vector2), axis=0)))
    ...
    def get_k_similar(self, data, vector):
        ...
        cosine_similarity = self.get_cosine_similarity(normalized_vector, normalized_data)
        ...
        index = cosine_similarity.nlargest(self.k).index
        # TODO: Return 2 values. See function comment
        return  index, cosine_similarity.nlargest(self.k)
    ...
    def get_rating(self, data, index, column):
        ...
        # classs
        similar_k_items = self.get_k_similar(new_data, vector)
        

        # TODO: Compute for the rating using the similarity values and the raw
        # ratings for the k similar items.
        
        rating = self.get_row_mean(((similar_k_items[1]*new_data.loc[similar_k_items[0], new_data.columns[column]]) / self.get_row_mean(similar_k_items[1])))
